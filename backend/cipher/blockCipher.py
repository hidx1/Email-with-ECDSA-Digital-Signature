...
'''
class BlockCipher:
    def set_initial(self, encrypt, input_name, is_file = False):
        if (is_file):
            ...
            if (encrypt):
                ...
                self.initial = Helper.convertHexToBinary64(input_name)
    
    def write_result(self, encrypt, file_name = None, is_file = False):
        if (is_file and file_name):
            Helper.convertBinary64ToFile(self.result, file_name)
            return True
        else:
            if (encrypt):
                return Helper.convertBinary64ToHex(self.result)
            else:
                return Helper.convertBinary64ToString(self.result)
